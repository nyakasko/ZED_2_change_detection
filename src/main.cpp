...
int main(int argc, char **argv) {
    ...
    float ratio_width = (float)((float)display_resolution.width / resolution.width);
    float ratio_height = (float)((float)display_resolution.height / resolution.height);
    ...
    changedetector.setZedParameters(init_parameters, calib_param_, display_resolution, ratio_width, ratio_height);
    // Start the main loop
#if show_pointcloud_in_pcl
    ...
    while (viewer.isAvailable() && !quit){
        ...
        if (zed.grab(runtime_parameters) == ERROR_CODE::SUCCESS) {
            ...
            // changedetector.show_object_detection_on_image(objects, image_zed_ocv, detection_confidence);
            detection_parameters_rt.detection_confidence_threshold = detection_confidence;
            ...
            // Comparing and displaying changes and matches between previously and currently detected objects
            changedetector.compare_for_change(min_, max_, PreviouslyDetectedObjects, cam_pose, image_zed_ocv, objects.object_list, DetectedObjects);
